...
| `ThemeVars.astro` | `{ theme }` → `<style>:root{…}</style>` com `--accent`, `--accent-2`, `--accent-soft`, `--accent-glow`, `--accent-text`, `--bg`, `--bg-2`, `--bg-3`, `--ink`, `--ink-2`, `--ink-3`, `--ink-4`, `--rule`, `--rule-2`, `--topbar`, `--radius`. |
